/// <reference types="node" />
import { DataItem, RegistryItem } from "@keystonehq/bc-ur-registry";
export declare class SolSignature extends RegistryItem {
    private requestId?;
    private signature;
    getRegistryType: () => import("@keystonehq/bc-ur-registry").RegistryType;
    constructor(signature: Buffer, requestId?: Buffer);
    getRequestId: () => Buffer | undefined;
    getSignature: () => Buffer;
    toDataItem: () => DataItem;
    static fromDataItem: (dataItem: DataItem) => SolSignature;
    static fromCBOR: (_cborPayload: Buffer) => SolSignature;
}
